...
def data_partition(
    ...
    data_dict = {
        speaker: group[['Filename', 'Path', 'Label', 'Utterance']].values.tolist()
        for _, (speaker, group) in enumerate(groups) 
        if len(group[['Filename', 'Path', 'Label', 'SeasondSpeaker', 'Utterance']]) > 10
    }
